...
public class CustomerAI : MonoBehaviour
{
    ...
    private CustomerAction ChooseAction()
    {
        ...
        do
        {
            ...
            attempts++;
            // max 10 tentatives, pour �viter une boucle infinie si toutes les actions sont �gales
        ...
        while (waitTime < maxWaitTime)
        {
            if (zone.HasFreeSpot())
            {
                ...
                if (zone.TryEnter(out spot))
                {
                    ...
                    stamina += staminaChange;
                    money -= moneyChange;
                    yield return new WaitForSeconds(Random.Range(3f, 8f)); // ou param�trable aussi si tu veux
                    zone.Exit(spot);
                    yield break;
                }
            }
